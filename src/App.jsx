            ...
            {sortedMonths.map(([k, v]) => (
              <div key={k} className="flex items-center gap-3">
                ...
                <div className="flex-1 bg-white/5 rounded-full h-2 overflow-hidden">
                  <div className="h-full bg-gradient-to-r from-violet-500 to-fuchsia-400 rounded-full" style={{ width: `${(v / maxMonth) * 100}%` }} />
                </div>
...
function ProfileTab({ user, onSignOut }) {
  ...
  const updateEmail = async () => {
    ...
    setLoading(false);
    showMsg(error ? error.message : "Confirmation sent ✓");
    setNewEmail("");
  };
